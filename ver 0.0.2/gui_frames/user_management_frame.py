...
try:
    # --- CORRECCIÓN PRINCIPAL: Añadir TODAS las constantes de config.py usadas ---
    from config import (
        ALL_DEFINED_ROLES, 
        ASSIGNABLE_ROLES_BY_SYSTEM_ADMIN, 
        ROLE_SUPERUSER, 
        SUPERUSER_INIT_USERNAME,
        ROLE_SYSTEM_ADMIN,  # Usado en la lógica de diálogos
        ROLE_DATA_MANAGER,  # Usado en la lógica de diálogos y mocks
        ROLE_STAFF_MEMBER,  # Usado en la lógica de diálogos y mocks
        UI_DEFAULT_FONT_FAMILY, # Usado en tk.Text en diálogos
        UI_DEFAULT_FONT_SIZE_NORMAL # Usado en tk.Text en diálogos
    )
...
class UserManagementFrame(ttk.Frame):
    ...
    def create_widgets(self):
        ...
        self.btn_volver_menu = ttk.Button(self.action_buttons_frame, text="Volver al Menu", command=lambda: self.controller.show_frame_by_name("MainMenuFrame"), style="TButton")
        self.btn_volver_menu.pack(side="right", padx=(0,5), pady=5) # Botón de volver al menú   
        self.selected_username = None
        self.selected_user_id = None        
